...
module.exports.setEvents = (ipcMain) => {
  ...
  ipcMain.handle('model.players.listFromTournamentByPoints', listFromTournamentByPoints)
...
async function listFromTournamentByPoints(event,tournament_uuid, orderings = []) {
  try {
    let players;
    let return_temporary_tournament_info = await generateTemporaryTournamentInfos(null,tournament_uuid);
    if(return_temporary_tournament_info.ok === 1){
      ...
      for(let player of players){
        ...
        console.log(player_return.temporary_tournament_info.slice(-1)[0].points);

        if(!i[player_return.temporary_tournament_info.slice(-1)[0].points]){
          i[player_return.temporary_tournament_info.slice(-1)[0].points] = 0;
        }

        if(!groups_return[String(player_return.temporary_tournament_info.slice(-1)[0].points)]){
          groups_return[String(player_return.temporary_tournament_info.slice(-1)[0].points)] = [];
        }

        groups_return[String(player_return.temporary_tournament_info.slice(-1)[0].points)][(i[player_return.temporary_tournament_info.slice(-1)[0].points]++)] = player_return;
      }
